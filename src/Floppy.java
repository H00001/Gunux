...
public class Floppy {
	...
	private final HashMap<Integer,ArrayList<ArrayList<byte[]>> > floppy = new HashMap<Integer,ArrayList<ArrayList<byte[]>> >(); //一个磁盘两个面
	...
	public void writeFileToFloppy(String fileName) {
		...
		try {
			...
			if (in.read(buf) != -1) {
				...
				this.writeFloppy(Floppy.MAGNETIC_HEAD.MAGNETIC_HEAD_0, 0, 1, buf);
			}
		} catch (IOException e) {
			e.printStackTrace();
		}
